...
const convertEnergy = (energy) =>
  energy.toLowerCase() == "fire"
    ? "R"
    : energy.toLowerCase() == "dragon"
      ? "X"
      : energy[0].toUpperCase();
const fixDescription = (text) =>
  text
    .replace(
      /<span class="energy-text energy-text--type-(\w+)"><\/span>/g,
      (_, type) => `{${convertEnergy(type)}}`,
    )
    .replace(/<br \/>/g, " ")
    .replace(/<.+?>/g, "")
    .replace(/\xA0/g, " ")
...
for (const card of card_data) {
  ...
  if (card.pokemon) {
    ...
    if (pokemon.previousEvolution) new_card.previousEvolution = pokemon.previousEvolution.name;
    ...
    if (pokemon.isUltraBeast) new_card.isUltraBeast = true;
